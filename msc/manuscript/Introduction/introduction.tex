...
However, the literature shows that alignment remains difficult to
achieve. Across policy and research studies, a key challenge is translating
high-level principles (e.g., transparency, fairness, safety, and
accountability) into practical and context-specific action within educational
settings. In practice, educators are required to interpret and enact policy in
...
This policy--practice gap is reflected in up-to-date studies.
...
evident in the study by \cite{bagdonaite2025artificial}, which shows that
enthusiasm for AI is accompanied by uncertainty about its pedagogical value,
...
that the problem is not one of innovation uptake alone, but of implementation
...
toward analysis of how policy is formulated, interpreted, communicated, and 
enacted. The policy--practice gap should be treated not as an 
administrative problem, but as a layered governance issue emerging across 
policy documents, institutional responses, and stakeholder discourse. 
This study therefore investigates the question:
...
separately, despite their interconnections. The value of a text-as-data
...
Using topic modelling, sentiment analysis, and selected causal-inference methods, the study sought to identify \textit{recurring themes}, detect
...
framework for explaining how the policy--practice gap emerges, where it
becomes visible, and how it is sustained across governance settings.
...
literature. Finally, Chapter~\ref{chap:conc} summarises the main findings,
