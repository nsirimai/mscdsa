...
The preceding causal NLP methods quantify semantic coverage and causal-frame divergence but do not
explain why policy and sentiment claims align or differ in meaning. The LLM-assisted stage will
therefore provide a separate explanatory analysis grounded directly in the shared cleaned sentence
inventory, independently of the outputs of the preceding methods. The analysis will concern
expressed causal or conditional meaning rather than real-world causal effects. Only original policy
and sentiment sentences will support substantive findings; synthetic sentiment will remain
restricted to robustness analysis.

The LLM-assisted stage is exploratory and is intended to demonstrate the potential of a systematic
agentic methodology for identifying and explaining evidence-grounded causal or conditional
relationships that may be difficult to capture with fixed NLP rules. A comprehensive application
would require deeper investigation across larger evidence sets, repeated runs, and extensive
verification, which could involve a large number of paid model tokens and computational cost. The
objective at this stage is not exhaustive gap discovery, but to establish the potential of combining
LLM reasoning, evidence grounding, independent verification, and cross-run stability in a
reproducible pipeline. Such an approach could extend conventional NLP analysis by supporting
interpretation of relationships that are difficult to encode through predefined rules alone. It does
not replace the causal NLP methods or use their outputs to generate findings. Comparison with the
semantic causal-claim coverage method in Subsection~\ref{subsec:method-semantic-coverage} will be
performed only after LLM findings have been generated, verified, and retained. This separation
allows the analysis to examine whether the LLM recovers expressed relationships that fixed cue-based
extraction does not retain.

For global and sufficiently supported country scopes, source-balanced evidence batches will be
constructed separately from policy and sentiment sources so that no single document dominates a
comparison. Broad causal-language candidates may be prioritised during sampling to increase the
availability of usable comparative evidence, but this sampling cue will not itself constitute
evidence of causality. Each sentence will retain its immutable identifier from the shared inventory,
allowing every finding to be traced to the same source sentence throughout analysis and
verification. Frozen topic identifiers may be attached after analysis for stratified diagnostic
reporting, but they will not influence evidence sampling, LLM interpretation, verification, or
finding retention.

The agent will determine whether sentences express a causal or conditional relationship, such as a
cause, prerequisite, risk, support, prevention, increase or decrease, or outcome. Thematic
similarity alone will not be sufficient. For comparable evidence, the agent will classify the
relationship as alignment, partial alignment, policy gap, sentiment gap, or insufficient evidence. A
policy gap indicates that policy expresses a stronger or different emphasis than sentiment, while a
sentiment gap indicates the converse. Each finding will contain a relation family, cause and effect
themes, an explanation, evidence and counterevidence identifiers, directional deficit scores,
evidence quality, direct comparability, and confidence. Alignment and insufficient-evidence outcomes
prevent a gap from being inferred from a comparison.

A second prompt will independently verify each proposed finding using the same evidence in reordered
form. Verification will check the evidence identifiers, whether the cited sentences express the
attributed relationship, direct policy--sentiment comparability, omitted counterevidence,
relation-family and theme consistency, and whether the explanation follows from the cited text.
Reordering will test whether the judgement depends on the evidence rather than its presentation
order. The verifier will also ensure that absence from a sampled batch is not interpreted as absence
from the corpus.

The verifier will additionally assign an evidence-quality category. Evidence is \textit{strong} when
both corpora directly express the same or closely corresponding causal relationship,
\textit{adequate} when both address the same relationship but one side is less explicit or differs
materially in cause, effect, or framing, and \textit{weak} when one side is only thematic,
descriptive, inferred from absence, or not genuinely causal. Weak findings will not proceed to
recurrent-finding analysis.

For each verified finding $f$, the verifier will also assign directional evidence
deficits
$\delta_{P\rightarrow S}(f)\in[0,1]$ and
$\delta_{S\rightarrow P}(f)\in[0,1]$. These are descriptive judgements over the
cited evidence rather than embedding or cosine distances. A value of zero indicates
that the source-side causal meaning is fully represented by the cited evidence from
the comparison corpus, whereas a value of one indicates very weak coverage or a
materially different causal meaning. The same interpretation is used in both
directions: values from $0.00$--$0.20$ indicate substantial correspondence,
$0.21$--$0.40$ mostly covered meaning, $0.41$--$0.60$ partial coverage,
$0.61$--$0.80$ substantial mismatch, and $0.81$--$1.00$ very weak coverage.
A high deficit cannot be inferred merely from the absence of evidence in a sampled
batch.
...
consistent findings across runs. Following~\cite{jaccard1901etude},
similarity between finite sets $A$ and $B$ is defined as
\begin{equation}
J(A,B)
=
\begin{cases}
...
The coefficients $0.50$, $0.40$, and $0.10$ are heuristic weights. Theme similarity
receives the largest weight because cause, effect, and gap meaning define the finding;
evidence overlap receives a comparable weight to keep matches grounded in similar
source sentences; and relation-family agreement receives a smaller weight because it
is a coarser signal and should not establish identity alone. The weights sum to one,
so $m(f,g)$ remains in $[0,1]$. Classification does not contribute to identity because
its agreement is evaluated separately after matching.
...
highest to lowest score, and a finding and recurrent cluster can each receive at
most one match from the same run. Unmatched findings begin new clusters.
...
$\phi_i\in[0,1]$ the evidence-faithfulness score of its verified run,
$a_i$ its classification, $r_i$ its relation family, and
$d_i\in\{0,1\}$ whether direct causal comparability was confirmed. Cluster-level
mean confidence and faithfulness are
\begin{equation}
\overline{c}(C)
=
...
The share of members with confirmed direct causal comparability is
$A_{\mathrm{comp}}(C)=k^{-1}\sum_{i=1}^{k}d_i$.
Thus, $\overline{c}(C)$ and $\overline{\phi}(C)$ measure mean confidence and
faithfulness, $A_{\mathrm{class}}(C)$ and $A_{\mathrm{rel}}(C)$ measure modal
classification and relation-family agreement, and $A_{\mathrm{comp}}(C)$ measures
the consistency of direct causal comparability across recurrent members.

The directional deficits reported for a recurrent cluster are the corresponding
means of the verified member scores:
...
adequate evidence quality and confirmed direct causal comparability. The modal
...
% \paragraph{Comparison with semantic causal-claim coverage.}
% The retained LLM findings will be compared with the causal-claim inventory generated
% by the fixed cue-based method in Subsection~\ref{subsec:method-semantic-coverage}.
% This comparison uses immutable sentence identifiers and does not influence LLM
...
% identifiers retained as eligible causal claims by the semantic-coverage method.
% For retained LLM finding $f$, let $P_f$ and $S_f$ denote its cited policy and
% sentiment evidence sets. The proportion of cited evidence omitted by fixed cue
% extraction is
...
% 1-
% \frac{
% |P_f\cap\mathcal{Q}_P|
% +
% |S_f\cap\mathcal{Q}_S|
% }{
% |P_f|+|S_f|
% }.
% \label{eq:method-agent-cue-omission}
% \end{equation}
...
% \mathbb{I}
% \left[
% P_f\cap\mathcal{Q}_P=\varnothing
% \;\lor\;
% S_f\cap\mathcal{Q}_S=\varnothing
% \right].
% \label{eq:method-agent-cue-missed}
% \end{equation}

% When $M_{\mathrm{cue}}(f)=1$, the finding is termed
% \textit{cue-inaccessible}: at least one side of the relationship is supported by
% LLM-cited causal or conditional evidence that was not retained by the predefined
% cue-based extraction. The proportion of retained LLM findings satisfying this
% condition is
...
% =
% \frac{1}{|\mathcal{F}|}
% \sum_{f\in\mathcal{F}}M_{\mathrm{cue}}(f),
% \label{eq:method-agent-cue-recovery-rate}
% \end{equation}

% where $\mathcal{F}$ denotes the set of retained empirical LLM findings.
% This measure will be used to examine the potential of the LLM-assisted approach to
...
% Findings for which $M_{\mathrm{cue}}(f)=0$ remain useful for a different comparison:
% both sides are available to semantic coverage, allowing the later analysis to examine
% whether numerical semantic proximity and LLM interpretation provide consistent or
% complementary accounts of the same evidence. These comparisons are descriptive and
% do not treat either method as ground truth. Their empirical results and substantive
% interpretation are reserved for Chapter~\ref{chap:analysis}.
...
``\textit{Teacher training reduces inappropriate use of generative AI}'', and the sentiment sentence
\texttt{S\_1}, ``\textit{Lack of training increases inappropriate use of generative AI}'', are both
retained by the rule-based method because they contain recognised causal cues. In that example,
\texttt{S\_1} is one of the three nearest sentiment claims to \texttt{P\_1}, and
Equation~\ref{eq:method-semantic-coverage} gives $C_{P\rightarrow S}^{(3)}(P_1)=0.667$ and
$D_{P\rightarrow S}^{(3)}(P_1)=0.333$. These measures quantify semantic proximity but do not explain
that the two sentences express compatible causal meaning through opposite framing: training is
associated with reduced misuse, whereas lack of training is associated with increased misuse. When
considered independently in an LLM evidence batch, the same sentences may therefore be classified as
\textit{alignment} or \textit{partial alignment}, with the interpretation grounded in the cited
sentences and traceable through their evidence identifiers.

The example also illustrates a relationship that fixed cue extraction may omit. Suppose the policy
evidence additionally contains \texttt{P\_3}, ``\textit{Responsible AI-assisted assessment depends
on clear institutional guidance}'', while the sentiment evidence contains \texttt{S\_5},
``\textit{Teachers hesitate to use AI when institutional guidance is unclear}''. The rule-based
extractor can retain \texttt{P\_3} through the recognised cue \textit{depends on}, whereas
\texttt{S\_5} would not satisfy the predefined cue inventory through its \textit{when} construction
alone. If both sentences occur in an LLM evidence batch, the agent may nevertheless recognise the
conditional relationship expressed by \texttt{S\_5} and compare it with \texttt{P\_3}, provided both
sentences directly support the interpretation.

In this case, \texttt{P\_3} is retained by the semantic-coverage method, whereas \texttt{S\_5} is
not, making the LLM finding cue-inaccessible because its sentiment evidence cannot enter the
semantic comparison. This does not establish correctness; the finding must still satisfy
verification, recurrence, evidence-quality, and human-validation requirements. Descriptive or
thematic sentences instead receive an insufficient-evidence classification. If interpretations of
\texttt{P\_3} and \texttt{S\_5} recur across runs, their match is evaluated using
Equation~\ref{eq:method-agent-match}, and qualifying findings form recurrent clusters measured by
Equation~\ref{eq:method-agent-stability}. The example illustrates how the LLM-assisted stage can
explain causal or conditional relationships beyond semantic similarity and recover relationships
outside fixed cue-based extraction. This separation preserves methodological independence between
the two approaches. It also supports transparent interpretation of complementary causal evidence.

For robustness analysis, policy evidence in each eligible batch will be held fixed while observed
sentiment evidence is replaced by source-balanced synthetic sentiment with comparable batch size and
source support. Synthetic outputs will remain separate from substantive findings. For paired
observed and synthetic packages, changes in classification and relation-family distributions will
use the total-variation form defined in Equation~\ref{eq:method-synthetic-robustness}, with finding
categories replacing topics.
...
$\Delta_{\mathrm{profile}}=1-J(H^{\mathrm{obs}},H^{\mathrm{syn}})$. Changes in confidence and
evidence faithfulness are
$\Delta\overline{c}=\overline{c}^{\,\mathrm{syn}}-\overline{c}^{\,\mathrm{obs}}$ and
$\Delta\overline{\phi}=\overline{\phi}^{\,\mathrm{syn}}-\overline{\phi}^{\,\mathrm{obs}}$, with
directional-deficit changes defined analogously. These measures assess sensitivity to
sentiment-evidence changes without treating synthetic outputs as substantive findings.

Final evaluation will compare machine-retained substantive findings with manually reviewed candidate
findings and report precision, recall, and $F_1$. The manual review will assess whether the cited
evidence supports the proposed causal or conditional comparison and whether the assigned finding
category is justified. Sampling parameters, thresholds, model settings, prompts, empirical results,
and the later cross-method comparison are reported in Section~\ref{chap:implresults} and
Chapter~\ref{chap:analysis}, respectively.}

\section*{Summary}

The methodology combines corpus construction, synthetic-data testing, three topic-modelling
approaches, two causal NLP methods, and an LLM-assisted analysis. LDA provides a lexical baseline,
BERTopic-style clustering a semantic grouping baseline, and NMF a factorisation-based representation
for topic aggregation and projection. Semantic causal-claim coverage measures semantic
correspondence, while causal-frame networks measure structural policy--sentiment divergence.
...
and sensitivity testing, and excluded from empirical inference. Generated topic labels
support interpretation without determining topic assignments. Country-level comparisons
are interpreted according to available evidence and source distribution, with limited
support treated cautiously rather than as representative. This separation preserves the
distinction between observed evidence and validation data. It also supports transparent
comparison across sources, countries, and analytical stages.

The causal NLP methods analyse expressed causal language and structure but do not establish
real-world causal mechanisms. Independently of their outputs, the LLM-assisted stage adds
evidence-grounded interpretation of causal or conditional policy--sentiment relationships through
verification and cross-run stability assessment. It complements rather than replaces quantitative
analysis or human validation. Reproducibility therefore requires preserving source metadata, cleaned
sentence inventories, model configurations, prompts, thresholds, evaluation outputs, and the
