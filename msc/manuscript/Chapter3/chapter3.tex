...
equal-size resamples of 761 claims, reducing dependence on one policy subset.
Following Subsection~\ref{subsec:method-semantic-coverage} and
Equation~\ref{eq:method-balanced-semantic}, policy-to-sentiment mean similarity was
0.6415 (deficit 0.3585), versus 0.6325 (deficit 0.3675) sentiment-to-policy, a
difference of \(0.0089\). Table~\ref{tab:impl-semantic-global} reports the outputs.
As reported in Table~\ref{tab:impl-causal-settings}, the inspection threshold was
\(\tau=0.511861\), the value below which the lowest 10\% of pooled empirical scores
fell. It selected low-coverage claims for review and closer manual inspection. The
audit contained 160 comparisons, but some records lacked a completed label stating
whether the paired claims matched. Threshold counts were therefore used only for
inspection, while similarity scores and deficits remained the pipeline measures. This
avoided treating an unchecked cut-off as evidence of accuracy. The percentile was a
screening rule, not a validated decision boundary. It flagged relatively weak matches
rather than classifying causal correctness.


As shown in Figure~\ref{fig:impl-semantic-results}(b), P8 (AI curriculum development
and school oversight) had the largest balanced policy-to-sentiment deficit at 0.3960,
followed by P0 (AI-assisted learning-assessment protocols) at 0.3763, P4 (cognitive
offloading in AI-assisted education) at 0.3729, and P1 (AI child protection and
rights) at 0.3677. In the full claim inventory, P8 contained 431 claims from 26
documents, P0 contained 604 claims from 29 documents, and P4 contained 211 claims
from six documents. P3 (AI competency-framework and curriculum development) had the
smallest policy-to-sentiment deficit at 0.3079. P8 and P0 therefore had broader
document support than P4. Among sentiment topics, S7 (online-learning intention
clarity and classroom relevance) had the largest balanced sentiment-to-policy deficit
at 0.4377, followed by S2 (GenAI detection and assessment confusion) at 0.4161 and S6
(public perceptions of AI and youth employment) at 0.4030. S7 and S2 drew on two
documents each, while S6 drew on three. {\color{blue} Table~\ref{tab:impl-semantic-native-topics}
lists all frozen NMF topics, their directional balanced deficits, and source support.
Higher deficits indicate weaker cross-corpus semantic counterparts.
Native policy and sentiment topic spaces remained separate.
These values identify comparatively weak semantic coverage within the corresponding
topic strata.}
...
(\texttt{CN2}, Table~\ref{tab:impl-repository-links}). It extracted cause,
relation, and effect spans, scored frames, projected spans into frozen policy NMF
space, and constructed weighted networks. Relations were normalised into six families:
\scalebox{0.8}{\texttt{causes\_or\_increases}},
\scalebox{0.8}{\texttt{reduces\_or\_prevents}},
\scalebox{0.8}{\texttt{enables\_or\_supports}},
\scalebox{0.8}{\texttt{requires\_or\_depends\_on}},
\scalebox{0.8}{\texttt{risks\_or\_threatens}}, and
\scalebox{0.8}{\texttt{expected\_to\_improve}}.
{\color{blue}Families were defined by shared causal meaning across cues.}
These grouped cues into relation types for network comparison.
The pipeline below summarises the workflow.
...
greater policy weight and orange greater sentiment weight, highlighting
...
These shares suggest different emphases in the extracted relation
families rather than differences in causal strength. Representative causal links
underlying these patterns are examined in Section~\ref{sec:analysis-gap}.
...
54--60 source documents. Larger gaps indicate greater differences in
how topics participate in extracted cause--effect links. Specific links contributing
to these differences are discussed in Section~\ref{sec:analysis-gap}.
...
sentiment sources. These values identify where structural differences
are most evident; their substantive meaning is considered in Section~\ref{sec:analysis-gap}.
...
Following Section~\ref{sec:method-llm-gap}, the implementation stage applied the LLM-assisted
pipeline identified as \texttt{LLM} in Table~\ref{tab:impl-repository-links}. The pipeline operated
directly on the cleaned sentence inventory rather than on the outputs of the causal NLP methods. Its
purpose was exploratory: to assess the potential of LLM-assisted reasoning alongside NLP techniques
for identifying and explaining source-grounded causal or conditional policy--sentiment
relationships. A comparison between the approaches is deferred to Chapter~\ref{chap:analysis}. This
design preserved traceability between interpretations and their supporting sentences.

The implemented workflow used separate analyst and verifier prompts. Evidence was sampled separately from policy and original sentiment sources for each scope, analysed for causal or
conditional relationships, and then verified using the same evidence in reordered form. Accepted
findings were matched across repeated runs and retained only when the recurrence and quality
requirements defined in Section~\ref{sec:method-llm-gap} were satisfied. Repeated execution also
provided evidence about the consistency of generated findings. Only findings surviving both
verification and recurrence checks entered final outputs.

{\small
\begin{verbatim}
Shared cleaned sentence inventory
  -> construct source-balanced policy and sentiment batches
  -> analyst LLM identifies evidence-grounded relationships
  -> verifier LLM rechecks evidence in reordered form
  -> repeat analysis across multiple runs
  -> match and cluster recurrent findings
  -> apply quality and stability filters
  -> retain verified empirical findings
  -> repeat with synthetic sentiment for robustness only
\end{verbatim}
}


The implementation used \texttt{deepseek-v4-pro} for both the analyst and verifier roles with
distinct prompts. Three evidence batches were created for each of the global, French, and Irish
scopes. Each batch contained 24 policy and 24 original sentiment sentences. The broad
causal-language indicator was used only to prioritise sampling and was not treated as evidence that
a causal relationship existed. Table~\ref{tab:impl-llm-settings} reports the executed settings and
thresholds using the notation defined in Section~\ref{sec:method-llm-gap}.

The nine empirical packages contained 432 sampled evidence records in total. Across them, 32
analysis attempts were required to obtain the target 27 usable runs. Verification retained 88
run-level findings, which were subsequently grouped into 58 cross-run candidate clusters. After
applying recurrence, confidence, faithfulness, agreement, stability, evidence-quality, and
direct-comparability requirements, 16 recurrent findings were retained for exploratory analysis.

Table~\ref{tab:impl-llm-findings} reports their distribution by scope. France produced eight
retained findings, while the global and Irish scopes each produced four. Thirteen findings were
classified as partial alignment, two as sentiment gaps, and one as alignment. No policy-gap finding
satisfied all retention criteria in this execution. The retained findings covered three relation
families. Nine concerned enabling or supporting relationships, five concerned risks or threats, and
two concerned requirements or dependencies. Evidence quality was assessed as strong for three
findings and adequate for 13. Mean classification agreement across repeated runs was 0.958 and mean
relation-family agreement was 0.927. Mean evidence-overlap stability was 0.795, while all retained
representative findings satisfied direct causal comparability.
...
\hline
\end{tabular}
\caption{Settings and retention thresholds of the LLM-assisted causal-gap implementation.}
...
The corresponding mean directional deficit was 0.428 from policy to sentiment and
0.470 from sentiment to policy. These values summarise the retained exploratory
findings rather than the entire corpora and should not be interpreted as estimates
of real-world causal effects or general policy--sentiment divergence.
...
Across the three scopes, the mean change in confidence was approximately 0.011 and
the mean change in evidence faithfulness was approximately $-0.013$. The synthetic
runs therefore provided a controlled sensitivity test without contributing
substantive evidence about stakeholder sentiment. Their limited changes also provide
an initial indication that the retained evaluation signals were not highly sensitive
to the synthetic substitution.
...
calculated at this stage. These measures will accompany the later evaluation once
the retained findings are manually reviewed. The corresponding human assessment and
its comparison with the automated findings are presented in
Chapter~\ref{chap:analysis}.

The LLM-assisted stage is treated here as an exploratory demonstration
of the potential of LLM reasoning for evidence-grounded causal-gap discovery. The
results show that candidate policy--sentiment relationships can be generated,
independently verified, matched across runs, and filtered according to
evidence and stability requirements. An exhaustive application would extend
this process across the full corpus rather than the sampled evidence batches used
here. Nevertheless, the retained findings illustrate the potential of
embedding LLM reasoning within a systematic causal-discovery workflow. They do not
establish that the LLM-assisted approach is superior to conventional causal NLP
techniques. The substantive interpretation of these findings and their direct
comparison with the semantic-coverage and causal-frame results are reserved for
Chapter~\ref{chap:analysis}.
...
requirements, demonstrating the feasibility of embedding evidence-grounded LLM
