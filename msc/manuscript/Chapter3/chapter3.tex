...
{\texttt{\detokenize{github.com/nsirimai/mscdsa/tree/main/msc/}}}:
...
remained separate from empirical evidence and was used for robustness and sensitivity
analysis. {\color{blue}The ChatGPT prompt referenced in
Table~\ref{tab:impl-repository-links} was compiled from European policy--sentiment
character imbalance and themes across corpora, then refined using lexical, semantic,
and classifier-based metrics}. It generated public-sentiment comments and
press-release-style Markdown texts covering AI literacy, teacher training, privacy,
trust, transparency, student support, responsible use, equity, and public confidence.
...
Subsection~\ref{subsec:method-chunking}, representing an entire report as one TF--IDF
vector, embedding, or topic-modelling input may merge unrelated subjects. The
implementation follows Hearst's segmentation approach \cite{hearst1997texttiling},
which divides documents into multi-paragraph chunks representing local subtopics. It
also reflects later work showing that representation quality depends on chunk length
and surrounding context \cite{gunther2024latechunking,duarte2024lumberchunker}.

The chunking pipeline retained Markdown headings as local context and preserved
source and chunk metadata. Chunks used a target length of 350 words with a 50-word
overlap and a minimum-size threshold. These values were study-specific design choices
rather than settings prescribed by the cited literature. The 350-word target retained
enough text for a short multi-paragraph argument without merging many unrelated
themes, while the 50-word overlap preserved continuity when an argument crossed a
chunk boundary without excessive repetition.
...
Evidence for improved comparability is provided in the next subsection, where
...
learning, guidance, strategy, and resources. These terms reflect
...
institutions. This indicates a focus on implementation, institutional
...
Other European sources. The exploratory grouping compared
...
As defined in Subsections~\ref{subsec:method-bertopic} and \ref{subsec:method-nmf},
BERTopic and NMF used the SLM \texttt{Qwen/Qwen2.5-1.5B-Instruct},
only after fitting. It generated labels from keywords and chunks,
which were reviewed before use. The SLM did not alter assignments, shares, scores, or
distributions. This kept topic discovery separate from label generation. The
following pipeline summarises this labelling procedure.
...
BERTopic and NMF fitted their empirical sentiment models only on original
...
topic definitions. It enabled comparison of topic coverage and shares,
showing which empirical themes were over-represented, under-represented, or absent in
the synthetic corpus. This isolates robustness effects from changes in empirical
topic discovery. The following pipeline summarises this robustness procedure.
...
matrices. Candidate counts were assessed using coherence and diversity. Policy models
were fitted globally and by country; sentiment corpora separately. Gensim used
\texttt{alpha="auto"} and \texttt{eta="auto"}, learning the document--topic and
topic--word priors from the corpus during fitting. LDA served as the baseline for
later topic-modelling pipeline comparisons. This provides a consistent lexical
reference for semantic models.

{\small
\begin{verbatim}
Policy or sentiment chunks
  -> select the corpus or country subset
  -> lowercase, tokenise, remove stopwords, and lemmatise retained tokens
...
\end{subfigure}
\caption{LDA coherence and diversity diagnostics for the policy (a),
original-sentiment (b), and synthetic-sentiment (c) models.}
...
Of 1,843 policy chunks, 1,724 were retained: 115 were removed because fewer than 30
processed tokens remained after cleaning and lemmatisation, and four exact
processed-text duplicates were excluded. The retained global model contained four
topics, with coherence of 0.7021 and diversity of 0.9000.
Figure~\ref{fig:impl-lda-searches} presents the candidate searches, while
Table~\ref{tab:impl-lda-policy-topics} lists the retained topics.

Country-specific models retained three topics for Australia, two for France and
Ireland, and seven for the USA. Principal terms varied across countries:

\begin{itemize}
  \item Australia: cognitive, privacy, assessment, risk;
  \item France: research, innovation, pedagogical deployment;
  \item Ireland: regulation, safety, assessment, literacy;
  \item USA: privacy, bias, equity, assessment, guidance.
\end{itemize}


For sentiment, the original corpus was modelled separately before synthetic data
were added. Figure~\ref{fig:impl-lda-searches} shows two topics for the
...
configuration combined TF--IDF, truncated Singular Value Decomposition (SVD),
K-means, and class-based TF--IDF. \cite{deerwester1990indexing} use SVD to reduce
high-dimensional representations to latent components. \cite{macqueen1967methods}
introduced K-means, which partitions vectors around $k$ centroids. K-means replaced
density-based clustering to provide fixed topic counts, complete assignments, stable
identifiers, reproducible country distributions, and synthetic projection.
Class-based TF--IDF produced representative terms. BERTopic supplied the
cluster-level representation, while transformer embeddings, UMAP, and HDBSCAN were
excluded.
...
The following pipeline summarises the BERTopic workflow from cleaned empirical
chunks to retained topics, reviewed labels, and exported distributions.
...
The following pipeline summarises this projection and comparison procedure.
...
metrics. Figure~\ref{fig:impl-bertopic-policy} also shows how retained topics vary
across countries.
...
Table~\ref{tab:impl-bertopic-policy-topics}. Figure~\ref{fig:impl-bertopic-policy}(a)
shows the candidate diagnostics, while Figure~\ref{fig:impl-bertopic-policy}(b)
shows the topic distribution by country. The labels were produced after clustering
from the keywords and representative chunks and did not affect the model outputs.
The global model supplied a shared country--topic matrix, while country models
produced local structures. Australia retained six topics, France eight, Ireland four,
and the USA seven. Table~\ref{tab:impl-bertopic-policy-countries} reports the main
country-level outputs used in later comparisons:
\begin{itemize}
  \item Australia: granular coverage of strategy, curriculum, training, integrity,
  and cognitive effects;
  \item France: strategy, curriculum, training, integrity, and cognitive effects;
  \item Ireland: concentrated on governance and digital-learning implementation;
  \item USA: assessment, tool oversight, teacher preparation, and privacy.
\end{itemize}
These outputs describe cross-country differences in corpus emphasis rather than
national policy maturity or effectiveness.
...
nine-topic solution had the highest coherence and combined score, with qualitative
review confirming clearer topic separation.
Figure~\ref{fig:impl-bertopic-sentiment-results}(a) shows selection diagnostics;
candidate metrics appear in Table~\ref{tab:impl-bertopic-sentiment-candidates}.
...
\hline
0 & Classroom AI use & 20.7\% & 7.9\% & $-12.8\%$ \\
1 & Teacher adaptation & 14.7\% & 0.0\% & $-14.7\%$ \\
2 & AI4T professional learning & 12.5\% & 11.9\% & $-0.6\%$ \\
3 & Ethics and governance & 11.6\% & 11.4\% & $-0.2\%$ \\
4 & Youth-sector risks and opportunities & 11.6\% & 15.8\% & $+4.2\%$ \\
5 & Child and youth social harm & 9.7\% & 0.0\% & $-9.7\%$ \\
6 & GenAI implementation concerns & 6.6\% & 19.8\% & $+13.2\%$ \\
7 & Irish public fears and hopes & 6.4\% & 15.3\% & $+8.9\%$ \\
8 & French public risks and benefits & 6.2\% & 17.8\% & $+11.6\%$ \\
\hline
\end{tabular}
\caption{Original and synthetic sentiment shares in the fitted BERTopic space; differences are in percentage points.}
...
check. The mean absolute difference between original and synthetic topic shares was
8.4 percentage points, and two sentiment topics received no synthetic assignments.
NMF was then implemented as a factorisation-based model to test whether the
representation provided broader thematic continuity.
...
document--topic and topic--term matrices. NNDSVDa initialisation, which fills zero
entries of NNDSVD with the mean, followed the SVD-based approach
of~\cite{boutsidis2008svd}. Coordinate descent was used to optimise the
matrices, as described by~\cite{hsieh2011fast}, while Frobenius loss measured
reconstruction error, using the NMF formulation of~\cite{lee1999learning}. A
seed of 42 and 1000 iterations were used. The following pipeline summarises the NMF
fitting and labelling workflow.
...
The pipeline below details synthetic projection into the retained NMF topic space.
For the synthetic check in Subsection~\ref{subsec:method-synthetic-robustness},
synthetic sentiment was transformed using the TF--IDF vectoriser and NMF model fitted
on original sentiment, then assigned to its highest-weight topic.
...
solution obtained the highest coherence and combined score and was retained. The
retained solution balanced statistical fit with thematic separation. Qualitative
review also supported nine topics: it separated privacy and ethical governance from
educator guidance and professional development, whereas eight merged these themes and
ten introduced further fragmentation. This selection preserved distinct policy themes
without over-fragmenting the corpus. The candidate results are shown in
Figure~\ref{fig:impl-nmf-policy} and Table~\ref{tab:impl-nmf-policy-candidates}.


The global NMF model supplied the country--topic matrix, while national models
captured local structures. Australia retained three topics, France seven, Ireland
four, and the USA seven. National models exposed local emphases hidden by global
aggregation within each national corpus. Table~\ref{tab:impl-nmf-policy-countries}
reports the country-level outputs. Country models therefore complemented rather than
replaced the global matrix. Australia showed the strongest concentration around
ethics guidance and staff training. Ireland was more evenly distributed across
teacher competence, child protection, public oversight, and academic integrity, while
France and the USA required broader seven-topic structures spanning pedagogy,
strategy, regulation, privacy, equity, and assessment. These patterns reflect corpus
emphasis rather than national policy quality or maturity.
...
Topic & Short label & Original & Synthetic & Difference \\
\hline
0 & Young people and well-being & 8.1\% & 7.9\% & $-0.2$ \\
1 & Student engagement and teacher adaptation & 15.8\% & 10.4\% & $-5.4$ \\
2 & GenAI assessment confusion & 7.3\% & 0.5\% & $-6.8$ \\
3 & Educator training and ethics & 19.8\% & 28.2\% & $+8.4$ \\
4 & Youth-worker perspectives & 12.1\% & 11.9\% & $-0.2$ \\
5 & Teacher experience and knowledge & 14.7\% & 9.4\% & $-5.3$ \\
6 & Public perception and employment & 8.6\% & 22.8\% & $+14.2$ \\
7 & Online-learning intention clarity & 7.3\% & 8.9\% & $+1.6$ \\
8 & French digital-technology views & 6.4\% & 0.0\% & $-6.4$ \\
\hline
\end{tabular}
...
causal-frame network outputs, and synthetic sensitivity checks. The broader
assessment of these results and their cross-method implications is reserved for
...
\hline
\end{tabular}
\caption{Shared cleaned sentence inventory and method-specific extraction scale.}\label{tab:impl-causal-scale}
\end{table}
...
Measures variation under weighting, source composition, quality, and projection rules.\\
\hline
\end{tabular}
\caption{Settings for the semantic-coverage and causal-frame pipelines.}
...
imported the cleaning module and verified the sentence-inventory hash. Preparation
normalised text, segmented sentences, removed source residue, and retained sentences
containing 6--110 words. Within-document duplicate sentences were removed before
either pipeline. The semantic workflow retained one claim per eligible sentence; the
frame workflow extracted relation occurrences. Cross-document repetition remained
available for corpus-level comparison. Source, document, chunk, and sentence
identifiers were preserved. Validation checked residue, duplicates, and
cleaning-version consistency. The shared inventory therefore provided a reproducible
basis for comparison. The pipeline below summarises preparation;
Tables~\ref{tab:impl-causal-scale} and~\ref{tab:impl-causal-settings} report units
and settings.

{\small
\begin{verbatim}
Combined chunk corpus
  -> normalise and repair extracted text
  -> segment chunks into sentence units
  -> remove source residue and invalid-length sentences
  -> remove within-document duplicate sentences
  -> assign source and corpus metadata
  -> validate and version the shared sentence inventory
  -> provide identical inputs to both causal pipelines
\end{verbatim}
}

All claims and frames retained source-sentence identifiers, preserving traceability
to the cleaned corpus. Shared preparation removed publication residue, while the
frame workflow removed repeated within-document relations. Accepted frames met a
minimum quality score of $0.40$, based on cue reliability, span completeness,
balance, content, and length described in
Subsection~\ref{subsec:method-frame-network}. Span-topic projection required
similarity of at least $0.02$ and a $0.003$ margin over the next-best topic. {\color{blue} These
thresholds were fixed before interpreting the policy--sentiment differences. The
quality threshold excluded weakly formed frames, while the projection thresholds
required minimum similarity and separation before direct topic assignment.} Uncertain
projections used the source-document topic fallback defined in
Subsection~\ref{subsec:method-frame-network}. Their influence was examined through
the frame-quality and projection specifications. These settings were not tuned to
maximise divergence. Fallback assignments remained identifiable in exported
diagnostics.
...
were calculated using Equation~\ref{eq:method-semantic-coverage}. As motivated in
Subsection~\ref{subsec:method-semantic-coverage}, a small neighbourhood reduces
dependence on a single close match while keeping the comparison local.
{\color{blue}This neighbourhood provided more than one semantic counterpart while
limiting averaging over increasingly weaker matches. It was fixed before interpreting
the results and applied in both directions. It avoids relying on one anomalous match.
Three neighbours preserve a local semantic comparison. Larger neighbourhoods could
dilute coverage with less similar claims.} Each neighbour contributed equally to the
coverage mean. Similarity was computed from normalised multilingual sentence
embeddings for cross-lingual semantic comparability across corpora. The pipeline
summarises the semantic-coverage workflow.

{\small
\begin{verbatim}
Shared cleaned sentence inventory
  -> retain the first valid auditable claim per eligible sentence
  -> encode claims with multilingual sentence embeddings
  -> compare three nearest opposite-corpus claims and calculate coverage deficits
  -> balance both directions through equal-size resampling
  -> aggregate results globally, by topic, and by country
  -> flag low-coverage claims using the inspection threshold
...
The inventories contained 3,930 policy and 761 sentiment causal claims, motivating 50
equal-size resamples of 761 claims, reducing dependence on one policy subset. As
reported in Table~\ref{tab:impl-semantic-global}, policy-to-sentiment mean similarity
was 0.6415 (deficit 0.3585), versus 0.6325 (deficit 0.3675) sentiment-to-policy, a
difference of \(0.0089\), following Subsection~\ref{subsec:method-semantic-coverage}
and Equation~\ref{eq:method-balanced-semantic}. As reported in
Table~\ref{tab:impl-causal-settings}, the inspection threshold was \(\tau=0.511861\),
the value below which the lowest 10\% of pooled empirical scores fell. It identified
low-coverage claims as candidates for later manual inspection. The audit contained
160 comparisons, but some records lacked a completed label stating whether the paired
claims matched. Threshold counts were therefore used only for inspection, while
similarity scores and deficits remained the pipeline measures. This avoided treating
the unchecked cut-off as evidence of accuracy. The percentile remained a screening
rule, not a validated decision boundary. Its substantive interpretation is discussed
further in detail in Subsection~\ref{subsec:analysis-semantic-coverage}.

{\color{blue} Table~\ref{tab:impl-semantic-native-topics} lists frozen NMF
topics, their directional balanced deficits, and source support. Higher deficits
indicate weaker cross-corpus semantic counterparts. Native policy and sentiment topic
spaces remained separate. The table identifies weak semantic
coverage across topic strata.} Figure~\ref{fig:impl-semantic-results}(b)
shows that P8 (AI curriculum development and school oversight) had the largest
balanced policy-to-sentiment deficit at 0.3960, followed by P0 (AI-assisted
learning-assessment protocols) at 0.3763, P4 (cognitive offloading in AI-assisted
education) at 0.3729, and P1 (AI child protection and rights) at 0.3677. In the full
claim inventory, P8 contained 431 claims from 26 documents, P0 contained 604 claims
from 29 documents, and P4 contained 211 claims from six documents. P3 (AI
competency-framework and curriculum development) had the smallest policy-to-sentiment
deficit at 0.3079. P8 and P0 had broader document support than P4. Among
sentiment topics, S7 (online-learning intention clarity and classroom relevance) had
the largest balanced sentiment-to-policy deficit at 0.4377, followed by S2 (GenAI
detection and assessment confusion) at 0.4161 and S6 (public perceptions of AI and
youth employment) at 0.4030. S7 and S2 drew on two documents each, while S6 drew on
three. The implications of these topic-level differences are examined in
Subsection~\ref{subsec:analysis-semantic-coverage}.
...
\hline
Topic & Reviewed NMF topic label & Deficit & Claims & Docs \\
\hline
P0 & AI-assisted learning-assessment protocols & 0.3763 & 604 & 29 \\
P1 & AI child protection and rights & 0.3677 & 275 & 17 \\
P2 & AI usage and pedagogy in school governance & 0.3533 & 192 & 5 \\
P3 & AI competency-framework and curriculum development & 0.3079 & 493 & 13 \\
P4 & Cognitive offloading in AI-assisted education & 0.3729 & 211 & 6 \\
P5 & Digital-learning frameworks and assessment policies & 0.3628 & 490 & 19 \\
P6 & AI ethical guidelines and school-data protection & 0.3639 & 683 & 41 \\
P7 & AI educator guidance and professional development & 0.3382 & 551 & 27 \\
P8 & AI curriculum development and school oversight & 0.3960 & 431 & 26 \\
\hline
S0 & Young people's AI impact on education and well-being & 0.3836 & 69 & 4 \\
S1 & Student engagement and teacher adaptation to AI & 0.3607 & 138 & 7 \\
S2 & GenAI detection and assessment confusion & 0.4161 & 49 & 2 \\
S3 & Educator training and ethical AI integration & 0.3400 & 220 & 8 \\
S4 & Youth workers' perspectives on AI integration & 0.3571 & 104 & 2 \\
S5 & Teachers' experience and knowledge in AI integration & 0.3641 & 81 & 5 \\
S6 & Public perceptions of AI and youth employment & 0.4030 & 46 & 3 \\
S7 & Online-learning intention clarity and classroom relevance & 0.4377 & 39 & 2 \\
S8 & French views on digital technologies, youth, and education & 0.3983 & 15 & 3 \\
\hline
\end{tabular}
\caption{Balanced directional semantic-coverage deficits and source support across
the frozen policy and sentiment NMF topics. Policy-topic rows report
\(P_x\rightarrow S\) deficits, while sentiment-topic rows report
\(S_x\rightarrow P\) deficits.}
\label{tab:impl-semantic-native-topics}
\end{table}

The country-level results are shown in
Figure~\ref{fig:impl-semantic-results}(c)--(d). France had its largest
policy-to-sentiment deficits for P0 and P6, although some cells contained claims from
only one to three documents. Ireland had broader source support, with P8 producing
its largest forward deficit and S7 its largest reverse-direction deficit. Empty cells
indicate that no eligible claims were available for the comparison.

Synthetic claims were projected into the original semantic analysis only as a
sensitivity check. Following Equation~\ref{eq:method-semantic-synthetic-change}, most
native sentiment-topic deficits changed by small amounts, while S2 changed by
approximately \(-0.057\). No synthetic claims were assigned to S8, so no change was
calculated for that topic. These separate synthetic results were interpreted further
in Subsection~\ref{subsec:analysis-semantic-coverage}.

\subsection{Causal-Frame Network Results}
\label{subsec:impl-frame-networks}

The causal-frame workflow followed Subsection~\ref{subsec:method-frame-network} (see
\texttt{CN2}, Table~\ref{tab:impl-repository-links}). It extracted cause, relation,
and effect spans, scored frames, projected spans into frozen policy NMF space, and
constructed weighted networks. Relations were normalised into six families:
\textit{causes or increases}, \textit{reduces or prevents},
\textit{enables or supports}, \textit{requires or depends on},
\textit{risks or threatens}, and \textit{expected to improve}.
{\color{blue}The families were predefined by grouping cues with shared causal meaning.}
These grouped cues into relation types for network comparison. The pipeline summarises
the workflow.
...
\begin{verbatim}
Shared cleaned sentence inventory
  -> detect English/French causal cues and extract cause-relation-effect spans
...
\(D_{\mathrm{JS}}=0.2596\). The source-balanced, high-quality-frame, and directly
projected-span specifications produced 0.2425, 0.2738, and 0.2964, respectively.
Across 250 source-level resamples, mean divergence was 0.3359, with a 2.5th--97.5th
percentile range of 0.2663--0.4157. This range reflects variation in source
composition rather than a confidence interval. Table~\ref{tab:impl-frame-robustness}
reports the four specifications. Divergence remained across specifications,
suggesting no weighting choice explained the result. The higher projected-only value
suggests that excluding fallback assignments increased network differences.
...
Panels~(a) and~(b) show policy and sentiment frame shares on the same
\(P0\)--\(P8\) cause--effect axes; sentiment spans are projected into this policy
topic space rather than retaining their native \(S0\)--\(S8\) assignments.
Panel~(c) shows signed share differences, where blue indicates greater policy
weight and orange greater sentiment weight.}
\label{fig:impl-frame-matrices}
\end{figure}

Document-topic fallback was used for approximately 31--35\% of cause and effect span
assignments, depending on corpus and span role. Exact zero-similarity
...
In practice, these patterns indicate which topic-to-topic causal links
...
for 3.8\% of policy weight and 4.1\% of sentiment weight. These shares suggest
different emphases in the relation families rather than differences in
causal strength. Causal links underlying these patterns are examined
in Subsection~\ref{subsec:analysis-causal-frame}.
...
\textit{requires or depends on} family produced the largest relation-gap change under
...
54--60 source documents. Larger gaps indicate greater differences in how topics
participate in extracted cause--effect links. Table~\ref{tab:impl-frame-gap-summary}
summarises the principal frame-gap and synthetic-sensitivity results. Specific links
contributing to these differences are discussed in Subsection~\ref{subsec:analysis-causal-frame}.
...
\hline
Scope & Leading result & Gap/divergence & Source support or comparison \\
\hline
Global & P0 & 0.3109 & 60 source documents \\
Global & P2 & 0.3099 & 54 source documents \\
Global & P8 & 0.2959 & 58 source documents \\
Global minimum & P3 & 0.1301 & Smallest topic-level gap \\
\hline
France & P2 & 1.6518 & 3 sentiment sources \\
Ireland & P7 & 0.5391 & 3 sentiment sources \\
Other & P3 & 0.7929 & Largest pooled-group gap \\
\hline
Original network & \(D_{\mathrm{JS}}\) & 0.2596 & Original sentiment \\
Synthetic network & \(D_{\mathrm{JS}}\) & 0.3611 & Change \(=+0.1015\) \\
Synthetic sensitivity &
\textit{requires or depends on} &
\(+0.3349\) &
Largest relation-gap change \\
\hline
\end{tabular}
\caption{Summary of principal causal-frame topic gaps, country-level differences,
and synthetic sensitivity results.}
\label{tab:impl-frame-gap-summary}
\end{table}

The topic-level measure \(G_t\) sums incoming and outgoing absolute differences and
is interpreted separately from the bounded Jensen--Shannon divergence.
Figure~\ref{fig:impl-frame-results}(c) shows country-level outputs for France,
Ireland, and the pooled Other European group. France was highest for P2
(\(1.6518\)) and P8, Ireland for P7 (\(0.5391\)), followed by P0 and P5
(\(\approx0.397\)), and the pooled group for P3 (\(0.7929\)). France and Ireland
each contained three original sentiment sources. These values locate the strongest
structural differences; their interpretation is deferred to
Subsection~\ref{subsec:analysis-causal-frame}.
...
Following Section~\ref{sec:method-llm-gap}, the implementation stage applied the
LLM-assisted pipeline identified as \texttt{LLM} in
Table~\ref{tab:impl-repository-links}. The pipeline operated directly on the cleaned
sentence inventory and did not use partial or complete outputs from the causal NLP
methods. Its purpose was exploratory: to assess the potential of LLM-assisted
reasoning alongside NLP techniques for identifying and explaining source-grounded
causal or conditional policy--sentiment relationships. Comparison between the
approaches is deferred to Chapter~\ref{chap:analysis}. This design preserved
traceability between interpretations and their supporting sentences.

The implemented workflow used separate analyst and verifier prompts. Evidence was
sampled separately from policy and original sentiment sources for each scope,
analysed for causal or conditional relationships, and then verified using the same
evidence in reordered form. Accepted findings were matched across repeated runs and
retained only when the recurrence and quality requirements defined in
Section~\ref{sec:method-llm-gap} were satisfied. Repeated execution also provided
evidence about the consistency of generated findings. Only findings surviving both
verification and recurrence checks entered final outputs. The pipeline below
summarises this workflow.

{\small
\begin{verbatim}
Shared cleaned sentence inventory
  -> sample source-balanced policy and sentiment batches by scope
  -> run analyst and verifier on each sampled batch
  -> repeat analysis across batches and multiple runs
  -> match and cluster recurrent findings across runs
  -> apply quality and stability filters
  -> retain verified empirical findings
  -> repeat batch sampling with synthetic sentiment for robustness only
\end{verbatim}
}


The implementation used \texttt{deepseek-v4-pro} for both the analyst and verifier
roles with distinct prompts. Three evidence batches were created for each of the
global, French, and Irish scopes. Each batch contained 24 policy and 24 original
sentiment sentences. The broad causal-language indicator was used only to prioritise
sampling and was not treated as evidence that a causal relationship existed.
Table~\ref{tab:impl-llm-settings} reports the executed settings and thresholds using
the notation defined in Section~\ref{sec:method-llm-gap}. The nine empirical packages
contained 432 sampled evidence records in total. Across them, 32 analysis attempts
were required to obtain the target 27 usable runs. Verification retained 88 run-level
findings, which were subsequently grouped into 58 cross-run candidate clusters. After
applying recurrence, confidence, faithfulness, agreement, stability,
evidence-quality, and direct-comparability requirements, 16 recurrent findings were
...
\hline
Empirical scopes & Global, France, Ireland \\
Batches per scope & 3 \\
Policy sentences per batch & 24 \\
Sentiment sentences per batch & 24 \\
Empirical analysis packages & 9 \\
Synthetic robustness packages & 9 \\
Target usable runs per package & 3 \\
Maximum attempts per package & 5 \\
Causal-hint sampling target & 0.85 \\
Minimum recurrent runs & 2 \\
Matching score $m(f,g)$ & $\geq 0.50$ \\
Mean confidence $\overline{c}(C)$ & $\geq 0.60$ \\
Mean faithfulness $\overline{\phi}(C)$ & $\geq 0.80$ \\
Classification agreement $A_{\mathrm{class}}(C)$ & $\geq 0.66$ \\
Relation agreement $A_{\mathrm{rel}}(C)$ & $\geq 0.50$ \\
Direct comparability $A_{\mathrm{comp}}(C)$ & $\geq 0.66$ \\
\hline
\end{tabular}
\caption{Settings and retention thresholds of the LLM-assisted causal-gap
implementation. The matching score, cluster-level confidence and faithfulness,
and classification and relation-family agreement follow
Equations~\ref{eq:method-agent-match}, \ref{eq:method-agent-cluster-means},
and~\ref{eq:method-agent-agreement}, respectively.}
\label{tab:impl-llm-settings}
\end{table}
...
Policy gap &
Sentiment gap &
Confidence $\overline{c}$ &
Faithfulness $\overline{\phi}$ &
Stability $S(C)$ \\
\hline
Global
& 4 & 1 & 3 & 0 & 0
...
\hline
\end{tabular}
\caption{Retained recurrent LLM-assisted findings by analytical scope. Confidence,
faithfulness, and stability are mean values for retained findings.}
\label{tab:impl-llm-findings}
\end{table}

Table~\ref{tab:impl-llm-findings} reports their distribution by scope. France
produced eight retained findings, while the global and Irish scopes each produced
four. Thirteen findings were classified as partial alignment, two as sentiment gaps,
and one as alignment. No policy-gap finding satisfied all retention criteria in this
execution. The retained findings covered three relation families. Nine concerned
enabling or supporting relationships, five concerned risks or threats, and two
concerned requirements or dependencies. Evidence quality was assessed as strong for
three findings and adequate for 13. Mean classification agreement across repeated
runs was 0.958 and mean relation-family agreement was 0.927. Mean evidence-overlap
stability was 0.795, while all retained representative findings satisfied direct
causal comparability. The corresponding mean directional deficit was 0.428 from
policy to sentiment and 0.470 from sentiment to policy. These values summarise the
retained exploratory findings rather than the entire corpora and should not be
interpreted as estimates of real-world causal effects or general policy--sentiment
divergence.

Manual-review fields were exported for the candidate findings, but no human labels
had been entered for this execution. Precision, recall, and $F_1$ were therefore not
calculated at this stage. These measures will accompany the later evaluation once the
retained findings are manually reviewed. The corresponding human assessment and its
comparison with the automated findings are presented in
Section~\ref{sec:analysis-llm-causal-gap}.

Following the synthetic-robustness procedure in Section~\ref{sec:method-llm-gap},
synthetic sentiment was evaluated separately through nine corresponding robustness
packages. Policy evidence remained fixed while original sentiment evidence was
replaced by source-balanced synthetic sentiment. The resulting outputs were retained
only as robustness diagnostics and were not included among the 16 empirical findings.
Using the mean confidence and faithfulness measures defined in
Equation~\ref{eq:method-agent-cluster-means}, the mean changes across the three
scopes were approximately \(\Delta\overline{c}=0.011\) and
\(\Delta\overline{\phi}=-0.013\), respectively. The synthetic runs therefore provided
a controlled sensitivity test without contributing substantive evidence about
stakeholder sentiment.

The LLM-assisted stage is treated here as an exploratory demonstration of the
potential of LLM reasoning for evidence-grounded causal-gap discovery. The results
show that candidate policy--sentiment relationships can be generated, independently
verified, matched across runs, and filtered according to evidence and stability
requirements. An exhaustive application would extend this process across the full
corpus rather than the sampled evidence batches used here. Nevertheless, the retained
findings illustrate the potential of embedding LLM reasoning within a systematic
causal-discovery workflow. They do not establish that the LLM-assisted approach is
superior to conventional causal NLP techniques. The substantive interpretation of
these findings and their direct comparison with the semantic-coverage and
causal-frame results are reserved for Section~\ref{sec:analysis-llm-causal-gap}.

\section*{Summary}

This chapter implemented the methodology of Chapter~\ref{chap:method} as an
incremental workflow from document preparation to topic modelling, causal NLP, and
LLM-assisted analysis. PDF extraction, Markdown cleaning, structured conversion, and
chunking preserved source provenance throughout. Synthetic sentiment remained
explicitly separated from empirical evidence and was used only for robustness and
sensitivity testing. Its validation showed substantial semantic proximity to the
original sentiment material while confirming that the generated texts remained
distinguishable.

Three complementary topic-modelling approaches were implemented. LDA provided the
lexical baseline, BERTopic-style clustering supplied complete cluster assignments,
and NMF provided a factorisation-based representation with fixed topic identifiers
and direct projection into the retained topic space. NMF was retained as the
reference topic space for causal analysis because of these operational properties
and its broader synthetic-topic coverage. Country-level outputs revealed differences
in corpus emphasis, while synthetic projections quantified the sensitivity of the
retained thematic structures without altering the empirical topic definitions.

The two causal NLP pipelines then operated independently on the same cleaned sentence
inventory. Semantic causal-claim coverage measured directional correspondence between
policy and sentiment claims, while causal-frame networks compared how topics
participated in extracted cause--effect structures. The balanced semantic deficits
were similar in both directions, whereas the frame analysis identified differences
in relation-family and topic-level structure. Synthetic and resampling checks were
used to assess the sensitivity of both pipelines. These outputs describe differences
in expressed causal structure rather than real-world causal mechanisms.

Finally, the LLM-assisted stage independently examined sampled policy and sentiment
evidence using separate analyst and verifier roles, repeated runs, and explicit
retention criteria. Sixteen recurrent findings survived verification and stability
requirements, illustrating the feasibility of embedding evidence-grounded LLM
reasoning within the causal-analysis workflow. An exhaustive application would extend
this process across the full corpus rather than sampled evidence batches. Human
evaluation, substantive interpretation, and direct comparison across the topic,
causal NLP, and LLM-assisted findings are reserved for
Chapter~\ref{chap:analysis}.